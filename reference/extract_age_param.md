...
Reads files matching the pattern "age_param_spatial_urban" in a
specified directory, extracts the country name, parameter values, and
optimization details, combines the results into a data frame, and
...
- output_file:
  ...
  A character string specifying the path to save the output data frame.
  If NULL, the output will not be saved.

## Value

A data frame with the extracted parameters, log-likelihood, and
...
params_df <- extract_age_param(dir_path = dummy_dir,
                               output_file = tempdir())
#> ✔ Model parameters extracted and saved to /tmp/Rtmpkz8QDq.
# }
```
