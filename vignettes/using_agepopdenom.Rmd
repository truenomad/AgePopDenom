...
If you do not (yet) have a DHS account, you can still run the entire workflow with simulated data: `simulate_dummy_dhs_pr()` writes a DHS-like dataset to the expected location, and a pre-simulated copy is shipped with the package (`system.file("extdata", "example_dhs_pr_gmb.rds", package = "AgePopDenom")`). See the *Example: Gambia* section below for a complete runnable script.
